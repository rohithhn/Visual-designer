/**
 * Image prompting engine: builds a structured visual brief from pasted content +
 * extracted heading/subheading/footer so image generation gets concrete,
...
function detectRedSignal(text: string): boolean {
  const lower = text.toLowerCase();
  return THREAT_SIGNALS.some(s => lower.includes(s));
}

/** Fallback brief when no raw content or when API call fails */
export function getFallbackVisualBrief(content: GeneratedContent): string {
...
SUPPORTING TEXT: Key concept label on hero element, 2–3 short annotation callouts supporting the message.`;
}

/**
 * Call LLM to generate a structured visual brief from the user's pasted content.
 * The brief includes mandatory fields: VISUAL TYPE, RED DECISION (from threat signal scan),
 * COMPOSITION (with creative surprise), SUBJECT, STYLE, PALETTE, SUPPORTING TEXT.
 * Used to give the image model precise, content-aware, creatively directed instructions.
 */
...
Do NOT include the heading, subheading, or footer text — they are displayed by the app separately.
...
/**
 * Build the content + visual direction block injected into the image prompt.
 * Surfaces the RED DECISION field prominently so the image model treats it
 * as a direct instruction, not a buried guideline.
 */
export function buildContentAndVisualBlock(
  content: GeneratedContent,
  visualBrief: string | null,
  omitContentTextInImage: boolean
): string {
  const contentSummary = `Heading: "${content.heading}". Subheading: "${content.subheading}". Footer: "${content.footer}".`;

  // Extract RED DECISION line from brief and surface it as a top-level instruction
  let redInstruction = "";
  if (visualBrief) {
  ...
  if (omitContentTextInImage) {
    ...
    return `TOPIC / MESSAGE TO SUPPORT:\n${contentSummary}\n\n*** DO NOT render the heading, subheading, or footer as text in the image. *** They are shown by the app elsewhere. Your image should include other text (labels, stats, severity badges, code fragments, callouts, step numbers) and any visuals (icons, illustrations, diagrams, charts).${direction}`;
  }
